...
export const getMe = asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  if (user.role === 'employee') {
    const permissions = await EmployeeModel.getPermissions(user.id);
    return res.json({ success: true, data: { ...data, permissions } });
  }

  res.json({ success: true, data: { ...data, permissions: null } });
});
...
export const getUserById = asyncHandler(async (req: AuthRequest, res: Response) => {
  const user = await UserModel.findById(parseInt(req.params.id as string, 10));
  if (!user) throw new AppError('User not found', 404);
  const { password: _pw, ...data } = user;
  res.json({ success: true, data });
});
...
export const deleteUser = asyncHandler(async (req: AuthRequest, res: Response) => {
  await UserModel.delete(parseInt(req.params.id as string, 10));
